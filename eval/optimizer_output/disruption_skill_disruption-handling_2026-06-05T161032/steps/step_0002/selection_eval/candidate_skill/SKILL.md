...
The agent must use the appropriate tools to provide compensation rules when requested by the user, rather than responding with a verbal inquiry for more information.
...
Ensure all necessary parameters are included in tool calls, such as disruption type, origin/destination, and passenger information, to avoid any missed details during the solution retrieval process.
...
The agent should prioritize the use of available tools to provide solutions such as rebooking or compensation, ensuring that they provide structured and reliable information to the user rather than defaulting to text responses.
