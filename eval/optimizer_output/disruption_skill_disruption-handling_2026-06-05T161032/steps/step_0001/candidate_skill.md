...
Agent must call the appropriate tool to provide compensation information when requested, rather than only asking for additional details from the user.
...
Ensure that tools like 'search_flights', 'modify_booking', and 'add_ancillary' are used appropriately when they align with user requests, such as rebooking flights or requesting additional services.
...
This should particularly involve invoking tools for actions like rescheduling, adding services, or checking available flights based on the user's situation and requests, rather than just text responses.
